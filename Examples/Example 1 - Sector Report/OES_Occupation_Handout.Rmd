...
SOC_CODE <- "29-1141" # Economists: "19-3011" # Statisticians: "15-2041"
```
...
<hr>
### Occupation Profile: `r oes$OCC_TITLE[oes$OCC_CODE == SOC_CODE][1]`

`r occ_desc$OCC_DESC[occ_desc$OCC_CODE == SOC_CODE]`
...
occ_state <- oes[oes$OCC_CODE == SOC_CODE & oes$JOBS_1000 != "**", ] %>%
  select(STATE, JOBS_1000) %>%
  left_join(data.frame(STATE = state.name), .) %>%
  mutate(state = tolower(STATE))
occ_state$JOBS_1000 <- as.numeric(occ_state$JOBS_1000)

map <- ggplot(occ_state, aes(map_id = state)) + 
...
gridExtra::grid.arrange(map, top5_states, nrow=1, widths=2:1)
...
occ_state <- oes[oes$OCC_CODE == SOC_CODE & oes$A_MEDIAN != "**", ] %>%
  select(STATE, A_MEDIAN) %>%
  left_join(data.frame(STATE = state.name), .) %>%
  mutate(state = tolower(STATE))
occ_state$A_MEDIAN <- as.integer(occ_state$A_MEDIAN)
...
gridExtra::grid.arrange(top5_states, map, nrow=1, widths=1:2)
